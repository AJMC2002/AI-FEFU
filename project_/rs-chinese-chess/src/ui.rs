use engine::board::{Board, Chess, Move, Player, Position, BOARD_HEIGHT, BOARD_WIDTH};
use fltk::{
    app,
...
pub fn ui(mut game: Board) -> anyhow::Result<()> {
    ...
    fn redrawn(group: &mut Group, game: &Board) {
        let possible_moves = game
            .generate_move(false)
            .iter()
            .filter_map(|m| if m.from == game.select_pos { Some(m.to) } else { None })
            .collect::<Vec<Position>>();
        for x_idx in 0..BOARD_WIDTH as usize {
            for y_idx in 0..BOARD_HEIGHT as usize {
                let chess = game.chesses[y_idx][x_idx];

                if chess.chess_type().is_some() {
                    let selected_chess = game.select_pos == (x_idx as i32, y_idx as i32).into();

                    let x = (x_idx + 1) * CHESS_SIZE - CHESS_SIZE / 2 - 24;
                    let y = (y_idx + 1) * CHESS_SIZE - CHESS_SIZE / 2 - 24;
                    let padding = 4;
                ...
                if possible_moves.contains(&(x_idx as i32, y_idx as i32).into()) {
                    let x = (x_idx + 1) * CHESS_SIZE - CHESS_SIZE / 2 - 24;
                    let y = (y_idx + 1) * CHESS_SIZE - CHESS_SIZE / 2 - 24;
                    let padding = CHESS_SIZE / 4 + CHESS_SIZE / 8;
                    let mut possible_pos = Frame::new(
                        (x + padding) as i32,
                        (y + padding) as i32,
                        (CHESS_SIZE / 4) as i32,
                        (CHESS_SIZE / 4) as i32,
                    ...
                    possible_pos.set_frame(FrameType::OvalBox);
                    possible_pos.set_color(if let Some(Player::Red) = game.chess_at(game.select_pos).player() {
                        Color::Red
                    } else {
                        Color::Blue
                    });

                    group.add(&possible_pos);
                }
            }
        }
    }
